#!/bin/bash
#
# Promote an already-uploaded app build to a wider audience — the beta testers
# or the general public — on iOS (App Store / TestFlight) and Android (Google
# Play).
#
# This is the canonical implementation, shared by all the apps; each app's
# promote.sh is a thin wrapper that sets the PROMOTE_* env below and execs
# this. Run the wrapper, not this file.
#
# It assumes a build was already uploaded (iOS: `ios/upload.sh` or CI put a
# build on the internal TestFlight track; Android: CI or `android/upload.sh`
# published to the Play internal track). It does NOT build or upload anything —
# it selects the latest uploaded
...
if [[ "$IOS" == 1 ]]; then
  # App Store Connect credentials. ios/secrets.env is gitignored and holds the
  # team/issuer ids plus a path to the key, which lives in ~/creds.
...
# --- preflight --------------------------------------------------------------
# In promote mode this asserts pubspec still describes the build that was
# actually uploaded. A commit between uploading and promoting bumps pubspec and
# quietly strands the build (it can no longer be promoted by version).
if [[ "$DRY_RUN" != 1 && -f "$SCRIPT_DIR/preflight.py" ]]; then
  echo "==> Preflight: checking versions against the stores..."
  PF_ARGS=(--promote)
  [[ "$ANDROID" == 0 ]] && PF_ARGS+=(--ios-only)
  [[ "$IOS" == 0 ]] && PF_ARGS+=(--android-only)
  ASC_BUNDLE_ID="$PROMOTE_BUNDLE_ID" \
    ...
    python3 "$SCRIPT_DIR/preflight.py" "${PF_ARGS[@]}"
fi
