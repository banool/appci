...
def configure(*, project_root, locale_dir, ios_targets, android_targets,
              poster_video_urls=None, post_create_avds=None):
    ...
    source is app-specific (bundled data file vs runtime dump). None means
    the app displays no videos, so no posters are prepared or served.
...
def drive(device_id):
    ...
    cmd = ["flutter", "drive", f"--driver={DRIVER}", f"--target={TARGET}",
           "-d", device_id]
    if device_id.startswith("emulator-") and POSTER_VIDEO_URLS is not None:
...
def main():
    ...
    if not args.ios_only:
        ...
        ensure_android_avds()
        poster_server = None
        if POSTER_VIDEO_URLS is not None:
            ensure_posters()
