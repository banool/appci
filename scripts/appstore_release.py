...
# Editable states where the version is still entirely ours — not yet handed to
# Apple — so its versionString can safely be changed to retarget the slot.
# WAITING_FOR_REVIEW is excluded on purpose: it is editable but already queued.
RENAMABLE_STATES = {
    "PREPARE_FOR_SUBMISSION",
    "DEVELOPER_REJECTED",
    "REJECTED",
...
def find_or_create_version(client, app_id, version_string, dry_run):
    ...
    # No version with this string. App Store Connect allows only ONE version in
    # an editable state at a time, so if a previous attempt left one behind
    # under a different number, creating ours fails with "You cannot create a
    # new version of the App in the current state". Renaming that record is the
    # supported way to retarget it, and it preserves the metadata already on it.
    # Deliberately excludes anything already with Apple (WAITING_FOR_REVIEW,
    # IN_REVIEW, ...) — those are in DONE_STATES and must not be touched.
    stale = next(
        (v for v in versions if version_state(v) in RENAMABLE_STATES),
        None,
    )
    if stale:
        stale_string = stale["attributes"].get("versionString")
        stale_state = version_state(stale)
        log(
            f"App Store version {stale_string} is sitting in {stale_state} and "
            f"holds the single editable slot; renaming it to {version_string}"
        )
        if dry_run:
            log("  [dry-run] would PATCH its versionString")
            return None, False
        st, data = client.call(
            "PATCH",
            f"/v1/appStoreVersions/{stale['id']}",
            body={
                "data": {
        ...
        if st in (200, 201):
            return stale["id"], False
        die(
            f"could not rename App Store version {stale_string} -> "
            f"{version_string}: {err_detail(data)}\n"
            f"  Delete the {stale_string} version in App Store Connect and retry."
        )

    # Nothing to reuse -> create it.
...
def submit_for_review(client, app_id, version_id, dry_run):
    ...
    # 0. Clear out a previous submission that failed review. A version stays
    #    attached to the submission that carried it, so while that submission
    #    sits in UNRESOLVED_ISSUES the version "cannot be reviewed" and every
    #    retry fails with a confusing "not in valid state". Cancelling it
    #    releases the version. Only ever touches UNRESOLVED_ISSUES — a
    #    submission actually queued or in review is left alone.
    st0, _, existing = client.get_all(
        "/v1/reviewSubmissions",
        params={"filter[app]": app_id, "filter[platform]": "IOS", "limit": "20"},
    )
    if st0 == 200:
        for sub in existing:
            if (sub.get("attributes") or {}).get("state") != "UNRESOLVED_ISSUES":
                continue
            log(f"  cancelling stale review submission {sub['id']} (UNRESOLVED_ISSUES)")
            stc, dc = client.call(
                "PATCH",
            ...
            if stc not in (200, 201):
                log(f"  WARNING: could not cancel it ({err_detail(dc)}); continuing anyway")
