#!/bin/bash
#
# Build and upload a Flutter app to TestFlight as an internal build, using
# xcodebuild and the App Store Connect API key.
#
# This is the canonical implementation, shared by all the apps; each app's
# ios/upload.sh is a thin wrapper that sets the UPLOAD_* env below and execs
...
# Fail before the archive if the store would reject this version anyway. Costs
# a couple of API calls; saves a ten-minute build and a wasted build number.
echo "==> Checking the version against the App Store..."
ASC_BUNDLE_ID="$UPLOAD_BUNDLE_ID" \
  APP_STORE_CONNECT_API_KEY_ID="$KEY_ID" \
  APP_STORE_CONNECT_API_ISSUER_ID="$APP_STORE_CONNECT_API_ISSUER_ID" \
  API_KEY_PATH="$API_KEY_PATH" \
  python3 "$SCRIPT_DIR/preflight.py" --ios-only
